...
# --- 纯压判定容差参数 (应力主导度比值法) ---
# EPSILON 定义了拉压比值的上限，0.1 表示只要拉应力不超过压应力的 10%，仍视为实质性受压
EPSILON = 0.1  
...
csv_output_path = os.path.join(output_folder, 'results_summary_SEI.csv')
...
for odb_file in odb_files:
    ...
    try:
        ...
        # --- 3. 核心子项计算（基于应力主导度比值法优化） ---
        # (1) 实质性纯压占比 Phi_comp
        v_pure_compression = 0.0
        
        for i in range(len(ivols)):
            s1 = s1_values[i]
            s3 = s3_values[i]
            
            # 满足以下两个条件之一，即判定为实质性受压：
            # 条件 1：绝对纯压 (s1 <= 0)
            # 条件 2：虽有微小拉应力 (s1 > 0)，但存在主压应力 (s3 < 0) 且拉压比值小于设定阈值 EPSILON
            if (s1 <= 0) or (s3 < 0 and (s1 / abs(s3)) < EPSILON):
                v_pure_compression += ivols[i]
        ...
        print("处理成功: %s | 实质纯压占比(ε=%s): %.2f%% | J: %.4f" % (odb_file, EPSILON, phi_comp*100, j_index))
